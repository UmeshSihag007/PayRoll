...
(function () {
	...
	//}
	function initActiveMenu() {
		// Assuming you have a baseURL variable that stores the base path of your Laravel application
		// For example, if your app is served from https://example.com/myapp/, baseURL should be '/myapp/'
		// If your app is served directly from the domain root, you might not need this
		var baseURL = window.baseURL || ''; // Adjust this as necessary

		// Get the path from the URL, excluding the base URL, query string, and hash
		var currentPath = window.location.pathname.replace(baseURL, '').split('?')[0].split('#')[0];

		// Adjust for root path
		if (!currentPath || currentPath === '/') currentPath = 'index';

		// Find the matching nav-link in the navbar-nav
		var navLinkSelector = `#navbar-nav a.nav-link[href$='${currentPath}']`; // Adjust selector as needed
		var activeLink = document.querySelector(navLinkSelector);
		...
		if (activeLink) {
			...
			var parentDropdowns = activeLink.closest('.collapse.menu-dropdown');
			while (parentDropdowns) {
				parentDropdowns.classList.add('show');
				var parentDropdownToggle = parentDropdowns.previousElementSibling;
				if (parentDropdownToggle) {
					parentDropdownToggle.classList.add('active');
					parentDropdownToggle.setAttribute('aria-expanded', 'true');
				}
				parentDropdowns = parentDropdownToggle ? parentDropdownToggle.closest('.collapse.menu-dropdown') : null;
			}
		}
	}
	function elementInViewport(el) {
